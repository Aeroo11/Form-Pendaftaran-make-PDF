...
def generate_pdf():
    try:
        # Try importing FPDF directly when needed
        ...
        if conn:
            ...
            # Query data
            query = """SELECT nama_lengkap, email, no_telepon, tanggal_lahir, 
                    asal_sekolah, jurusan_pilihan, nilai_rata_rata 
                    FROM pendaftar ORDER BY waktu_daftar DESC"""
            cursor.execute(query)
            siswa_list = cursor.fetchall()
            close_connection(conn, cursor)
            ...
            pdf.cell(0, 10, 'UNIVERSITAS TEKNOLOGI INDONESIA', 0, 1, 'C')
            ...
            col_widths = [10, 50, 40, 30, 30, 30]
            headers = ['No', 'Nama Lengkap', 'Email', 'Telepon', 'Tgl Lahir', 'Jurusan']
            ...
            for i, siswa in enumerate(siswa_list):
                ...
                pdf.cell(col_widths[1], 10, siswa['nama_lengkap'], 1)
                pdf.cell(col_widths[2], 10, siswa['email'], 1)
                pdf.cell(col_widths[3], 10, siswa['no_telepon'], 1)
                
                # Format tanggal
                tanggal = siswa['tanggal_lahir']
                if isinstance(tanggal, datetime.date):
                    tanggal = tanggal.strftime('%d-%m-%Y')
                pdf.cell(col_widths[4], 10, str(tanggal), 1)
                
                pdf.cell(col_widths[5], 10, siswa['jurusan_pilihan'], 1)
                pdf.ln()
        ...
        flash('PDF generation is not available. Please install the required packages: pip install fpdf2')
        return redirect(url_for('daftar_siswa'))
    except Exception as e:
        flash(f'Terjadi kesalahan saat membuat PDF: {str(e)}')
        return redirect(url_for('daftar_siswa'))
